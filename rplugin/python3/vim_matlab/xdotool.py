...
class Xdotool:
    ...
    def find_windows_by_class(self, window_class):
        result = self.run("xdotool search --maxdepth 4 --class {}".format(window_class))
        window_ids = result.strip().split()

        if len(window_ids) == 0:
            raise RuntimeError('Unable to find window class "{}"'.format(window_class))

        return window_ids

    def find_windows_by_name(self, window_name):
        result = self.run('xdotool search --maxdepth 4 --name "{}"'.format(window_name))
        window_ids = result.strip().split()

        if len(window_ids) == 0:
            raise RuntimeError("Unable to find window name {}".format(window_name))
    ...
    def find_windows(self, window_name, window_class, is_unique=True):
        ...
        if is_unique and len(result) > 1:
            raise RuntimeError(
                "More and one window found: {}, {}".format(window_name, window_class)
            )
